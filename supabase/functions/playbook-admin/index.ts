...
function pickChars(chars: string, count: number): string[] {
  ...
  return Array.from(bytes, (value) => chars[value % chars.length]);
}
...
async function resetUserPassword(
  ...
  if (!targetUserId) throw new Error("Usuario alvo ausente.");
  if (targetUserId === callerUserId) {
    throw new Error("A conta administrativa atual nao pode resetar a propria senha por este painel.");
  }
  ...
  const updateAuthResult = await admin.auth.admin.updateUserById(targetUserId, {
  ...
  const auditResult = await admin
    ...
    .insert({
      ...
      old_role: target.role,
      new_role: target.role,
      notes: toText(body.note) || "Temporary password generated by administrator.",
    });
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === "reset_password") {
      return await resetUserPassword(admin, body, caller.user.id);
    }
